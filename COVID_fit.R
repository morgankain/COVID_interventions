...
nparams           <- 20               ## number of parameter sobol samples (more = longer)
...
gg.fit <- mifs_local %>%
  traces() %>%
  melt() %>%
  filter(variable == "loglik" | variable == "beta0" | variable == "soc_dist_level_sip") %>% 
