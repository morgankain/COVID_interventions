sir_step_mobility <- Csnippet("
                ...
                //   double betat;
                     ...
                     if ((beta0_k_post > 0) & (I > 0)) {
                      if ((catch_eff_post > 0) & intervention == 1) {
                        betat = rtgamma_eff(n_beta, sqrt(beta0 / beta0_k_post), beta0*dt/d, beta_catch_post, catch_eff_post)*exp(log(beta_min)*sip_prop_post);
                      } else {
                        betat = rgammawn(sqrt(beta0 / (beta0_k_post * I)), beta0*dt/d) * exp(log(beta_min)*sip_prop_post);                
                      }
                     } else {
                        betat = beta0*dt/d*exp(log(beta_min)*sip_prop_post);
                     }    
...
check_R0   <- function (beta0est, beta_min, fixed_params, sd_strength, prop_S, desired_R) {
  ...
  log(desired_R / (beta0est * inf)) / log(beta_min)
